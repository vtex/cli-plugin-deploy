...
import { CustomCommand, ToolbeltConfig, ColorifyConstants } from 'vtex'
...
export default class Deploy extends CustomCommand {
  static description = `Publishes an app as a stable version. Only works for apps previously published as a release candidate [see ${ColorifyConstants.COMMAND_OR_VTEX_REF(
    'vtex publish --help'
  )}].`

  static examples = [
    `${ColorifyConstants.COMMAND_OR_VTEX_REF('vtex deploy')}`,
    `${ColorifyConstants.COMMAND_OR_VTEX_REF('vtex deploy')} vtex.service-example@0.0.1`,
  ]
  ...
  static flags = {
    ...
    force: oclifFlags.boolean({
      char: 'f',
      description: `Ignores the testing period of 7 minutes after publishing an app. ${chalk.yellow(
        '(Use with caution.)'
      )}`,
    }),
  }
